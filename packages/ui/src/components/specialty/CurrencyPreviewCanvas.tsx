...
export interface CurrencyAdjustedLayout {
  frameWidth: number;
  frameHeight: number;
...
interface CurrencyPreviewCanvasProps {
  ...
  adjustedLayout: CurrencyAdjustedLayout;
  selectedFrame: FrameStyle;
  topMatColor: Mat;
  bottomMatColor: Mat;
  backingColor: CurrencyBackingOption;
...
export const CurrencyPreviewCanvas = memo(function CurrencyPreviewCanvas({
  layout,
  framePhotos,
  adjustedLayout,
  selectedFrame,
  ...
  const topMatHex = topMatColor.hexColor ?? "#FFFFFF";
  const bottomMatHex = bottomMatColor.hexColor ?? "#000000";
  ...
  const renderOpening = (bevelColor: string) => (
    ...
    >
      <CurrencyIcon className="w-16 h-12 text-muted-foreground/30" />
    </div>
  );

  const renderSingleMatOpening = (bevelColor: string) => (
    <div
      style={{
        position: "absolute",
  ...
  const innerContent = (
    ...
    >
      {matType === "none" ? (
        <div style={backingOnlyStyle}>
          <CurrencyIcon className="w-16 h-12 text-muted-foreground/30" />
        </div>
      ) : matType === "single" ? (
        <div style={matLayerStyle(topMatHex, topMatColor.name)}>
          {renderSingleMatOpening(getMatBevelColor(topMatColor.name))}
          {renderBrassPlaque()}
        </div>
      ) : (
        <div style={matLayerStyle(topMatHex, topMatColor.name)}>
          ...
          >
            {renderOpening(getMatBevelColor(bottomMatColor.name))}
          </div>
          {renderBrassPlaque()}
        </div>
      )}
  ...
  return (
    <div className={className} style={style} data-testid="currency-preview-canvas">
      {hasFramePhotos ? (
        <div
          style={{
